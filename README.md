...
`fscramble` is a *simple* tool for scrambling files. It takes a directory and randomly scrambles their contents, writing the result to a specified output directory. Its main purpose is to rapidly simulate changes in userdata over a longer time periods. Alternatively, it can be used to create a collection of randomized files.

# Requirements
- You need to have the [Java development kit](https://www.oracle.com/java/technologies/downloads/#java21) installed. Java 24 is not yet supported so you might need to downgrade to 21.
- This project uses [Gradle](https://gradle.org/install/) as its build tool.
...
# Usage
`fscramble` accepts cli parameters or a configuration file. It accepts three modes:
* `scramble [<options>]` - Scramble input files to a specific directory. Options are:
...
* `create [<options>]` - Create files from scratch. Options are:
    ...
    * (conditional) `--count <num>`: The amount of files to create. If present, a flat number of files will be created. Conflicts with `--size`.
    ...
    * (optional) `--file-types <types>`: What files (specified by extension) to create. By default, all available file types are created. You can get a list of supported file types by running `fscramble --list-file-types`.
