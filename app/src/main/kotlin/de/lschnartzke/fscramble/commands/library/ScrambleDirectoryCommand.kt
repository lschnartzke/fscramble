...
class ScrambleDirectoryCommand : CliktCommand() {
    val dataDirectory: String by option("-d", "--data-directory", help="Data directory to source sample data from").required().validate {
        File(it).isDirectory
    }

    val inpath: Path by option("-i", "--inpath", help="Directory to scramble").path().required()
    ...
    override fun run() {
        ...
        lib.scrambleDirectory(inpath, outpath, scrambleCount)
    }
}